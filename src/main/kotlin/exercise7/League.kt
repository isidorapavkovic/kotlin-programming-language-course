...
internal class League(override val teams: List<Team>, private val fixtures: List<Fixture>) : LeagueApi {
    ...
    private fun createLeagueEntries(matches: List<Match>, team: Team): LeagueTableEntry {
    ...
    private fun matchesOfTeams(scorerTeam: Team, againstTeam: Team) = fixtures
        .flatMap { it.matches }
        .filter { match: Match ->
            (scorerTeam == match.homeTeam && againstTeam == match.awayTeam) ||
                    (scorerTeam == match.awayTeam && againstTeam == match.homeTeam)
        }
